...
module.exports = function(options) {
  ...
  resources[prefixed('FailedTaskPlacementMetric')] = {
    ...
    Properties: {
      FilterPattern: 'watcher task did not run',
  ...
  resources[prefixed('InvocationErrorMetric')] = {
    Type: 'AWS::Logs::MetricFilter',
    Properties: {
      FilterPattern: 'watcher outcome failed',
      LogGroupName: cf.ref(prefixed('LogGroup')),
      MetricTransformations: [{
  ...
  resources[prefixed('ApproximateReceivesCount')] = {
    Type: 'AWS::Logs::MetricFilter',
    Properties: {
      FilterPattern: '[timestamp, workerwatcher=[watchbot], logtype, started=started, task=task, for=for, attempt=attempt, attemptnumber, ...]',
      LogGroupName: cf.ref(prefixed('LogGroup')),
      MetricTransformations: [{
        MetricName: cf.join(['ApproximateReceivesCount-', cf.stackName]),
        MetricNamespace: 'Mapbox/ecs-watchbot',
        MetricValue: "$attemptnumber"
      }]
    }
  };

  resources[prefixed('MaxConcurrency')] = {
    Type: 'AWS::Logs::MetricFilter',
    Properties: {
      FilterPattern: '[timestamp, workerwatcher=[watchbot], logtype, concurrency=concurrency, maxconcurrency, ...]',
      LogGroupName: cf.ref(prefixed('LogGroup')),
      MetricTransformations: [{
        MetricName: cf.join(['MaxConcurrency-', cf.stackName]),
        MetricNamespace: 'Mapbox/ecs-watchbot',
        MetricValue: "$maxconcurrency"
      }]
    }
  };

  resources[prefixed('RunningWorkersPerWatcher')] = {
    Type: 'AWS::Logs::MetricFilter',
    Properties: {
      FilterPattern: '[timestamp, workerwatcher=[watchbot], logtype, concurrency=concurrency, maxconcurrency, pipe=|, tasks=tasks, numworkers, ...]',
      LogGroupName: cf.ref(prefixed('LogGroup')),
      MetricTransformations: [{
        MetricName: cf.join(['RunningWorkersPerWatcher-', cf.stackName]),
        MetricNamespace: 'Mapbox/ecs-watchbot',
        MetricValue: "$numworkers"
      }]
    }
  };

  resources[prefixed('WorkerDuration')] = {
    Type: 'AWS::Logs::MetricFilter',
    Properties: {
      FilterPattern: '[..., duration=duration:, time, s=s]',
      LogGroupName: cf.ref(prefixed('LogGroup')),
      MetricTransformations: [{
        MetricName: cf.join(['WorkerDuration-', cf.stackName]),
        MetricNamespace: 'Mapbox/ecs-watchbot',
        MetricValue: "$time"
      }]
    }
